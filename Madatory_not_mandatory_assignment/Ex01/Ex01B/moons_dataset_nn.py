...
# plot_2d_separator(lr, X_train, fill=True, alpha=.3)
...
# pyplot.scatter(X[:, 0], X[:, 1], c=labels, cmap=cm3, s=50)
...
# plot_2d_separator(dt_classifier, X, fill=True, alpha=.3)
# discrete_scatter(X[:, 0], X[:, 1], y)

# Create a Random Forest Classifier
#for i in range(1,10):
    # rf_classifier = RandomForestClassifier(random_state=42, max_depth=10, n_estimators=i)
    # rf_classifier.fit(X, y)
    #
    # # Plot the decision boundary
    # plot_2d_separator(rf_classifier, X, fill=True, alpha=.3)
    # discrete_scatter(X[:, 0], X[:, 1], y)
    # pyplot.show()
...
svm_poly3 = SVC(kernel='poly', degree=3, C= 100)
...
plt.subplot(2, 2, 3)
plot_2d_separator(svm_poly3, X, fill=True, alpha=.3)
discrete_scatter(X[:, 0], X[:, 1], y)
